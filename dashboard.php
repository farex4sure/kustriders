    ...
    <div class='flex h-screen flex-col'>
                    ...
                    <div class='flex w-full justify-between items-center px-3 py-2 md:py-4 md:px-6 bg-white bg-opacity-75'>
                        <div class='flex w-full gap-2'>
                            <div class='flex flex-col w-full justify-between gap-1 items-start'>
                                ...
                                <?php
                                $lastpay = "SELECT * FROM monthly_pay WHERE userid='".$_SESSION['loggedin_id']."'";
                                $r = $conn->query($lastpay);
                                if ($r->num_rows > 0) {
                                    while($row = $r->fetch_assoc()) {
                                        // $id = $row['id'];
                                        $lpay = $row['lastpay'];
                                        $npay = $row['nextpay'];
                                        $npay=date("d M, Y", $npay);
                                        $lpay=date("d M, Y", $lpay);
                                    }
                                }
                                ?>
                                <p class=''><?php echo $lpay ?></p>
                            ...
                            <div class='flex flex-col w-full justify-between gap-1 items-end'>
                                ...
                                <p class=''><?php echo $npay ?></p>
                                <button data-modal-toggle="defaultModal" type="button" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-7 rounded">
                                    Pay now
                                </button>
                            </div>
                        </div>
        ...
        <footer class='flex w-full justify-center p-3 px-0 border-t border-1 bg-green-500'>
            <div class='flex justify-around w-full max-w-5xl'>
                ...
                <span class='text-green-600 text-lg md:text-3xl'>
                    <a href="profile.php"><i class='text-white fa fa-user'></i></a>
                </span>
                <span class='text-green-600 text-lg md:text-3xl'>
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 space-y-6">
                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                    The Next Day of Payment Would be <?php echo $npay ?>
                </p>
            </div>
            <!-- Modal footer -->
            <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                <button data-modal-toggle="defaultModal" type="button" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Ok</button>
            </div>
        </div>
    </div>
</div>
